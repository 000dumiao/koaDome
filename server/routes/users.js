...
const {apiKey, scope, emailSMTP, myEmail, clientId, telId, clientSecret, tokenSecret} = require('../config')
...
router.post('/findPwd', async ctx => {
  ...
  if (user) {
    ...
    let mailOptions = {
      from: 'dumiaomiao '+ myEmail, // 从哪个邮箱发送
      to: email,
      subject: '找回密码', // 标题
...
router.post('/sendMsg', async ctx => {
  ...
  let options = {
    ...
    qs: {
      mobile: phone,
      tpl_id: telId,
      content: `【杜三水】您的验证码为${code},如非本人操作,请忽略此条短信。该验证码5分钟之内有效,感谢您对我们的支持。`,
      key: apiKey
    }
  }
  try {
